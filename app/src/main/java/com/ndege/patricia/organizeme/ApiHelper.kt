package com.ndege.basil_empapp
...
class ApiHelper(var context: Context) {
    ...
    //PUT
    fun put(api: String, jsonData: JSONObject, param: CallBack){
        Toast.makeText(context, "Please Wait for response", Toast.LENGTH_LONG).show()
        val client = AsyncHttpClient(true, 80, 443)
        val con_body = StringEntity(jsonData.toString())
        //PUT to API
    ...
    //DELETE
    fun delete(api: String, jsonData: JSONObject, callback: CallBack){
        Toast.makeText(context, "Please Wait for response", Toast.LENGTH_LONG).show()
        val client = AsyncHttpClient(true, 80, 443)
        val con_body = StringEntity(jsonData.toString())
        //DELETE to API
        client.delete(context, api, con_body, "application/json",
            object : JsonHttpResponseHandler(){
                override fun onSuccess(
                    ...
                    Toast.makeText(context, "Response ${response?.getString("message")} ", Toast.LENGTH_SHORT).show()
